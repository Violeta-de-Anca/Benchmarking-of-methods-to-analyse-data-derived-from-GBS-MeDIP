...
list.p.values.loop.1000 <- foreach(i = 1:1000, .combine = function(...) rbindlist(list(...), use.names = F), .packages = c("data.table","edgeR")) %dopar% {
  ...
  top.RBC.bonfe=topTags(like.ratio.RBC, n=10000, adjust.method = "bonferroni")
  
  # Combine list.mann.whitney.RBC into a data table
  list(top.RBC.bonfe$table$FWER)
}
list.p.values.loop.1000=unlist(list.p.values.loop.1000)
save(list.p.values.loop.1000,file = "/proj/naiss2024-23-57/GBS_MeDIP_benchmark/benchmark_final_tables/list.1000.p.values.loop.wolf_ML_bonferroni.rda")
stopCluster(cl)
