...
list.p.values.loop.1000 <- foreach(i = 1:1000, .combine = function(...) rbindlist(list(...), use.names = F), .packages = c("data.table","edgeR")) %dopar% {
  ...
  top.quasi.RBC=quasi.like.ratio.RBC$table$PValue
  list(top.quasi.RBC)
  #top.quasi.RBC=topTags(quasi.like.ratio.RBC, n=10000, adjust.method = "fdr")
...
save(list.p.values.loop.1000,file = "/proj/naiss2024-23-57/GBS_MeDIP_benchmark/benchmark_final_tables/list.1000.p.values.loop.chicken_QL.rda")
#save(list.p.values.loop.1000,file = "/proj/naiss2024-23-57/GBS_MeDIP_benchmark/benchmark_final_tables/list.1000.p.values.loop.chicken_QL_FDR.rda")
stopCluster(cl)
