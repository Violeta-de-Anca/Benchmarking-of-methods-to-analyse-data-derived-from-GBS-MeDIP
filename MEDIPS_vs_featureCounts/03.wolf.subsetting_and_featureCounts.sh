...
#SBATCH -t 10:00:00
#SBATCH -J wolf.3
#SBATCH --error /proj/naiss2024-23-57/GBS_MeDIP_benchmark/log_files/wolf_count_matrix.err
#SBATCH --output /proj/naiss2024-23-57/GBS_MeDIP_benchmark/log_files/wolf_count_matrix.out
...
aligned_dir=$working_dir/datasets/wolf
count_dir=$working_dir/benchmark_final_tables
saf_dir=$working_dir/datasets/wolf/merged

# First and additionally one can also filter by properly paired and quality map > 10
file_list=($aligned_dir/*_reheader.bam)
for file in "${file_list[@]}"; do
	name=${file##*/}
	x=${name%_sorted_reheader.bam}
...
featureCounts -F SAF -p -B --countReadPairs -o $count_dir/count.matrix.wolf.bed -a $saf_dir/final_sorted.saf -d 0 -D 1500 -T 1 $aligned_dir/*.MQ10.bam
